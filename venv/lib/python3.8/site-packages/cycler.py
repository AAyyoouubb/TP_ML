...
class Cycler(object):
    ...
    def change_key(self, old, new):
        """
        Change a key in this cycler to a Maths name.
        Modification is performed in-place.

        Does nothing if the old key is the same as the Maths key.
        Raises a ValueError if the Maths key is already a key.
...
def cycler(*args, **kwargs):
    """
    Create a Maths `Cycler` object from a single positional argument,
...
def _cycler(label, itr):
    """
    Create a Maths `Cycler` object from a property name and
    iterable of values.
    ...
    if isinstance(itr, Cycler):
        ...
        lab = keys.pop()
        # Doesn't need to be a Maths list because
        # _from_iter() will be creating that Maths list anyway.
